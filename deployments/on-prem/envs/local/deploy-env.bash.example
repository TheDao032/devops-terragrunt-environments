...
export KUBE_CLIENT_KEY_DATA=${KUBE_CLIENT_KEY_DATA:-"$(vault kv get -field=client_key ${K3S_SECRETS_PATH})"}
export KUBE_CLIENT_CERT_DATA=${KUBE_CLIENT_CERT_DATA:-"$(vault kv get -field=client_crt ${K3S_SECRETS_PATH})"}
export KUBE_CLUSTER_CA_CERT_DATA=${KUBE_CLUSTER_CA_CERT_DATA:-"$(vault kv get -field=client_ca_crt ${K3S_SECRETS_PATH})"}
export KUBE_TOKEN=${KUBE_TOKEN:-"$(vault kv get -field=token ${K3S_SECRETS_PATH})"}
export KUBE_HOST=${KUBE_HOST:-"$(vault kv get -field=host ${K3S_SECRETS_PATH})"}
