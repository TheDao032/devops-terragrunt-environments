...
# cluster platform — Vault, CRDs and the External-Secrets operator are cluster-wide and already
# deployed by LOCAL's init-resources. Prod does NOT stand up its own, so this unit is ALWAYS
# excluded (guards against `run --all` deploying a second Vault). Re-enable only for a dedicated
# prod cluster.
exclude {
  if      = true
  actions = ["all"]
...
inputs = {
  ...
  vault_conf = {
    ...
    common = {
      ...
      host                  = "vault.k3s.prod"   # Traefik ingress host + cert SAN
    }
