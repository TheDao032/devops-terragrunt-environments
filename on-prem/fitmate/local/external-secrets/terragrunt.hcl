locals {
  ...
  vault_config = {
    ...
    vault_token             = base64encode(local.vault_config_vars.locals.vault_token)
...
terraform {
  source = "../../../../../devops-terraform-modules//on-prem/shared/external-secrets"
  # source = "git::git@github.com:TheDao032/devops-terraform-modules.git//on-prem/sharedexternal-secrets?ref=${local.environment}"
}
...
inputs = {
  eso_role = dependency.vault-auth.outputs.roles["external-secrets"]
