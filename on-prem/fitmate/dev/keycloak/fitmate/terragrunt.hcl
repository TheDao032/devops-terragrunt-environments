locals {
  ...
  keycloak_url = get_env("KEYCLOAK_URL", "http://keycloak.k3s.dev")
}
...
exclude {
  if = run_cmd("--terragrunt-quiet", "bash", "-c",
    "curl -fs -o /dev/null --max-time 3 $${KEYCLOAK_URL:-http://keycloak.k3s.dev}/realms/master/.well-known/openid-configuration && curl -fs -o /dev/null --max-time 3 $${VAULT_ADDR:-http://vault.k3s.dev}/v1/sys/health && echo false || echo true"
  ) == "true"
  actions = ["all"]
}
...
inputs = {
  ...
  realm = {
    ...
    ssl_required = "none" # HTTP lab: Keycloak reached at http://keycloak.k3s.dev via Traefik
