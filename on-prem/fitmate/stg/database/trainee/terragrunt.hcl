...
inputs = {
  ...
  services = {
    trainee = {
      ...
      grants = [
        { role = "trainee_ro_${local.environment}", schema = "app", object_type = "schema", privileges = ["USAGE"] },
        # SELECT on FUTURE tables trainee_app_${local.environment} creates (i.e. migration tables) — default privilege.
        { role = "trainee_ro_${local.environment}", schema = "app", object_type = "table", privileges = ["SELECT"], on_future = true, owner = "trainee_app_${local.environment}" },
      ]
    }
  }
}
